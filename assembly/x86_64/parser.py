#!/usr/bin/env python3
"""
Assembly Parser for GAMBA MBA Automation
Parses assembly files extracted from IDA into structured instruction data
"""
...
class Instruction:
    ...
    def __init__(self, address: int, mnemonic: str, operands: List[str], 
                 disasm: str, bytes_hex: str = "", size: int = 0):
...
def parse_assembly_file(asm_file: Path) -> List[Instruction]:
    """
    Parse assembly file extracted from IDA.
    ...
    for line_num, line in enumerate(lines, 1):
        ...
        # Parse operands (split by comma, handle brackets)
        ...
        inst = Instruction(
            ...
            bytes_hex="",  # Not available in extracted assembly
...
def parse_operands(operands_str: str) -> List[str]:
    ...
    - Memory references with brackets: [rax+rbx*4]
    - Constants: 0x1234, 58h
...
def parse_assembly(asm_file: Path) -> Dict:
    """
    Parse assembly file and return structured data.
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python assembly_parser.py <assembly_file.asm>")
        sys.exit(1)
